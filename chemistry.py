...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    #   Multiply the atomic mass by the quantity.
        element_atomic_mass = atomic_mass * element[1]

    #   Add the product into the total mass.
        total_mass += element_atomic_mass
    return element_atomic_mass



def main():
    formula = input('\nWhat is the molecular forumula?\n')

    periodic_table = make_periodic_table()
    symbol_quantity_list = parse_formula(formula, periodic_table)
    print(compute_molar_mass(symbol_quantity_list, periodic_table))
